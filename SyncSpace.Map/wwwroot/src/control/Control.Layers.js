...
export const Layers = Control.extend({
	...
	expand() {
		this._container.classList.add('map-control-layers-expanded');
		this._section.style.height = null;
		const acceptableHeight = this._map.getSize().y - (this._container.offsetTop + 50);
		if (acceptableHeight < this._section.clientHeight) {
			this._section.classList.add('map-control-layers-scrollbar');
			this._section.style.height = `${acceptableHeight}px`;
		} else {
			this._section.classList.remove('map-control-layers-scrollbar');
		}
	...
	collapse() {
		this._container.classList.remove('map-control-layers-expanded');
		return this;
	},

	_initLayout() {
		const className = 'map-control-layers',
	...
	_createRadioElement(name, checked) {
		const radioHtml = `<input type="radio" class="map-control-layers-selector" name="${name}"${checked ? ' checked="checked"' : ''}/>`;
	...
	_addItem(obj) {
		...
		if (obj.overlay) {
			...
			input = this._createRadioElement(`map-base-layers_${Util.stamp(this)}`, checked);
		}
