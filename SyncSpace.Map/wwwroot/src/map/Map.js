...
export const Map = Evented.extend({
    ...
    panBy(offset, options) {
        ...
        if (options.animate !== false) {
            this._mapPane.classList.add('map-pan-anim');
    ...
    createPane(name, container) {
        const className = `map-pane${name ? ` map-${name.replace('Pane', '')}-pane` : ''}`,
    ...
    _initLayout() {
        ...
        if (Browser.touch) { classes.push('map-touch'); }
        if (Browser.retina) { classes.push('map-retina'); }
        if (Browser.safari) { classes.push('map-safari'); }
        if (this._fadeAnimated) { classes.push('map-fade-anim'); }
    ...
    _initPanes() {
        ...
        if (!this.options.markerZoomAnimation) {
            panes.markerPane.classList.add('map-zoom-hide');
            panes.shadowPane.classList.add('map-zoom-hide');
        }
    },
    ...
    _onPanTransitionEnd() {
        this._mapPane.classList.remove('map-pan-anim');
        this.fire('moveend');
    },
    ...
    _createAnimProxy() {
        const proxy = this._proxy = DomUtil.create('div', 'map-proxy map-zoom-animated');
    ...
    _animateZoom(center, zoom, startAnim, noUpdate) {
        ...
        if (startAnim) {
            ...
            this._mapPane.classList.add('map-zoom-anim');
        }
    ...
    _onZoomTransitionEnd() {
        ...
        if (this._mapPane) {
            this._mapPane.classList.remove('map-zoom-anim');
        }
