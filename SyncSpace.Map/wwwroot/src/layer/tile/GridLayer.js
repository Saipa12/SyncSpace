 ...
 *         var tile = L.DomUtil.create('canvas', 'map-tile');
 *
...
export const GridLayer = Layer.extend({
	...
	_initContainer() {
		...
		this._container = DomUtil.create('div', `map-layer ${this.options.className || ''}`);
		this._updateZIndex();
	...
	_updateLevels() {
		...
		if (!level) {
			...
			level.el = DomUtil.create('div', 'map-tile-container map-zoom-animated', this._container);
	...
	_tileReady(coords, err, tile) {
		...
		if (!err) {
			tile.el.classList.add('map-tile-loaded');
